...
pub struct Simulation {
    ...
    pub tick_count: u64,
    pub history_log: Vec<String>,     // public for WASM access
    rng: StdRng,                      // private, but helper methods expose functionality

    // reusable buffers
    mate_candidates: Vec<(usize, usize, usize)>,   // (index, x, y)
    cell_agents: Vec<Vec<usize>>,
    mutation_scale: f32,
    density_grid: Vec<u8>,           // per‑cell agent counts for input #10
}
...
impl Simulation {
    ...
    pub fn force_ice_age(&mut self) {
        // Align tick_count to the start of an Ice Age period
    ...
    pub fn tick(&mut self) {
        ...
        // --- shuffle agent processing order to eliminate bias ---
        ...
        // common sine wave for map phase (input 8)
        ...
        // --- perception and movement (using shuffled order) ---
        for &i in &order {
            let agent = &self.agents[i];
            let vision = agent.genome.vision_range(self.config.max_vision);
            let v = vision as i32;

            // nearest food (toroidal search)
        ...
        // --- aggression (borrow‑safe, O(1) victim selection) ---
